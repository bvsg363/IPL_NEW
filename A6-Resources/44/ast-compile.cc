...
Code_For_Ast & Call_Ast::compile(){
	...
	for(list<Ast *>::iterator it = actual_param_list.begin(); it != actual_param_list.end(); it++)
    {
		...
		string s = (**form_param_itr).get_variable_name();

		Symbol_Table_Entry *sym = new Symbol_Table_Entry(s, (**form_param_itr).get_data_type(), lineno, sp_ref);
		sym->set_start_offset(offset);
		// sym->set_symbol_scope(local);

		Ics_Opd *result = new Mem_Addr_Opd(*sym);
		...
		argcfa.get_reg()->reset_use_for_expr_result();
	}

	if(offset != 0){
		cfa.append_ics(*(new Compute_IC_Stmt(sub, stack_opd, new Const_Opd<int>(-offset), stack_opd)));
	}
	...
	if(offset != 0){
		cfa.append_ics(*(new Compute_IC_Stmt(add, stack_opd, new Const_Opd<int>(-offset), stack_opd)));
	}
